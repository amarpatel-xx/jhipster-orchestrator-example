...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('orchestratorgatewayApp.error', (response: EventWithContent<unknown> | string) => {
    ...
    this.httpErrorListener = this.eventManager.subscribe(
      'orchestratorgatewayApp.httpError',
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(
        `orchestratorgatewayApp.${fieldError.objectName as string}.${convertedField}`,
      );
