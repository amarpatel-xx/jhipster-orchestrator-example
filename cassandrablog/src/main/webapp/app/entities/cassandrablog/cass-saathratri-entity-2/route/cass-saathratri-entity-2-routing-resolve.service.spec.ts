...
import cassSaathratriEntity2Resolve from './cass-saathratri-entity-2-routing-resolve.service';

describe('CassSaathratriEntity2 routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICassSaathratriEntity2 returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          cassSaathratriEntity2Resolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should navigate to 404 when find returns an empty body', async () => {
      ...
      service.find = vitest.fn(() => of(new HttpResponse<ICassSaathratriEntity2>({ body: null })));
      mockActivatedRouteSnapshot.params = { entityTypeId: 'val-1', yearOfDateAdded: 'val-2', arrivalDate: 'val-3', blogId: 'val-4' };

      // WHEN
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(cassSaathratriEntity2Resolve(mockActivatedRouteSnapshot))).rejects.toThrowError(
