...
import { ICassSaathratriEntity4, ICassSaathratriEntity4Id } from '../cass-saathratri-entity-4.model';
import { CassSaathratriEntity4DeleteDialogComponent } from '../delete/cass-saathratri-entity-4-delete-dialog';
import { CassSaathratriEntity4Service, EntityArrayResponseType, EntityResponseType } from '../service/cass-saathratri-entity-4.service';

@Component({
  selector: 'jhi-cass-saathratri-entity-4',
  templateUrl: './cass-saathratri-entity-4.html',
  imports: [
    RouterLink,
    FormsModule,
    FontAwesomeModule,
...
export class CassSaathratriEntity4Component implements OnInit {
  subscription: Subscription | null = null;
  readonly cassSaathratriEntity4s = signal<ICassSaathratriEntity4[]>([]);
  ...
  protected readonly cassSaathratriEntity4Service = inject(CassSaathratriEntity4Service);
  ...
  // Saathratri: Composite Primary Key Code
  trackCompositeId = (item: ICassSaathratriEntity4): ICassSaathratriEntity4Id =>
    this.cassSaathratriEntity4Service.getCassSaathratriEntity4Identifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (this.cassSaathratriEntity4s().length === 0) {
            this.load();
          }
        }),
      )
  ...
  delete(cassSaathratriEntity4: ICassSaathratriEntity4): void {
    const modalRef = this.modalService.open(CassSaathratriEntity4DeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.cassSaathratriEntity4 = cassSaathratriEntity4;
  ...
  resetAndLoad(): void {
    ...
    this.cassSaathratriEntity4s.set([]);
    this.load();
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType, append = false): void {
    ...
    if (append) {
      const current = this.cassSaathratriEntity4s();
      ...
      this.cassSaathratriEntity4s.set(this.refineData([...current, ...newItems]));
    } else {
      this.cassSaathratriEntity4s.set(this.refineData(dataFromBody));
    }
    if (this.hasNextPage && this.totalItems !== null) {
      this.hasNextPage = this.cassSaathratriEntity4s().length < this.totalItems;
    }
  }

  protected refineData(data: ICassSaathratriEntity4[]): ICassSaathratriEntity4[] {
    ...
    return data.sort((a, b) => {
      ...
      if (compositeKeyFields.includes(predicate)) {
        aVal = a.compositeId[predicate as keyof ICassSaathratriEntity4Id];
        bVal = b.compositeId[predicate as keyof ICassSaathratriEntity4Id];
      } else {
  ...
  protected fillComponentAttributesFromResponseBody(data: ICassSaathratriEntity4[] | null): ICassSaathratriEntity4[] {
    return data ?? [];
  }
  ...
  private getEntityKey(item: ICassSaathratriEntity4): string {
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.isSearchActive) {
      if (
        ...
        return this.cassSaathratriEntity4Service
        ...
        return this.cassSaathratriEntity4Service.findAllByCompositeIdOrganizationIdPageable(
          this.searchCriteria.organizationId,
          queryObject,
        );
      }
    ...
    return this.cassSaathratriEntity4Service.querySlice(queryObject);
  }
