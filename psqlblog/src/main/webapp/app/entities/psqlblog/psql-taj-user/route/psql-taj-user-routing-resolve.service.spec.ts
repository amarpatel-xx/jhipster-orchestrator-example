...
import psqlTajUserResolve from './psql-taj-user-routing-resolve.service';

describe('PsqlTajUser routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPsqlTajUser returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          psqlTajUserResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(psqlTajUserResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('9fec3727-3421-4967-b213-ba36557ca194');
