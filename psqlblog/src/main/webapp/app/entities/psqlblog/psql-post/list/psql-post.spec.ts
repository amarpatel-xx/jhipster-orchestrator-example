...
import { sampleWithRequiredData } from '../psql-post.test-samples';
import { PsqlPostService } from '../service/psql-post.service';

import { PsqlPost } from './psql-post';

vitest.useFakeTimers();

describe('PsqlPost Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PsqlPost;
  let fixture: ComponentFixture<PsqlPost>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PsqlPost);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'd5e9bde4-66e1-4c16-a5c8-29f7c8766362' }], {
    ...
    expect(comp.psqlPosts()[0]).toEqual(expect.objectContaining({ id: 'd5e9bde4-66e1-4c16-a5c8-29f7c8766362' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'd5e9bde4-66e1-4c16-a5c8-29f7c8766362' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.psqlPosts()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'd5e9bde4-66e1-4c16-a5c8-29f7c8766362' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to psqlPostService', () => {
      ...
      vitest.spyOn(service, 'getPsqlPostIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPsqlPostIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.psqlPostsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
