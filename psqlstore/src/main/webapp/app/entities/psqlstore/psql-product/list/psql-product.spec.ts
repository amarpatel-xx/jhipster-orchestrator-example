...
import { sampleWithRequiredData } from '../psql-product.test-samples';
import { PsqlProductService } from '../service/psql-product.service';

import { PsqlProduct } from './psql-product';

vitest.useFakeTimers();

describe('PsqlProduct Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PsqlProduct;
  let fixture: ComponentFixture<PsqlProduct>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PsqlProduct);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '27c299d6-3e62-4e45-b862-4c6b629af87a' }], {
    ...
    expect(comp.psqlProducts()[0]).toEqual(expect.objectContaining({ id: '27c299d6-3e62-4e45-b862-4c6b629af87a' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '27c299d6-3e62-4e45-b862-4c6b629af87a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.psqlProducts()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '27c299d6-3e62-4e45-b862-4c6b629af87a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to psqlProductService', () => {
      ...
      vitest.spyOn(service, 'getPsqlProductIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPsqlProductIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.psqlProductsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
