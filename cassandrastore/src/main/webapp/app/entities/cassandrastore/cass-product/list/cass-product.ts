...
import { CassProductDeleteDialogComponent } from '../delete/cass-product-delete-dialog';
import { CassProductService, EntityArrayResponseType } from '../service/cass-product.service';

@Component({
  selector: 'jhi-cass-product',
  templateUrl: './cass-product.html',
  imports: [
    RouterLink,
    FormsModule,
    FontAwesomeModule,
...
export class CassProductComponent implements OnInit {
  subscription: Subscription | null = null;
  readonly cassProducts = signal<ICassProduct[]>([]);
  ...
  protected readonly cassProductService = inject(CassProductService);
  ...
  // Saathratri: Single-value Primary Key Code
  trackId = (item: ICassProduct): string => this.cassProductService.getCassProductIdentifier(item);
  ...
  delete(cassProduct: ICassProduct): void {
    const modalRef = this.modalService.open(CassProductDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.cassProduct = cassProduct;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType, append = false): void {
    ...
    if (append) {
      const current = this.cassProducts();
      ...
      this.cassProducts.set(this.refineData([...current, ...newItems]));
    } else {
      this.cassProducts.set(this.refineData(dataFromBody));
    }
    if (this.hasNextPage && this.totalItems !== null) {
      this.hasNextPage = this.cassProducts().length < this.totalItems;
    }
  }

  protected refineData(data: ICassProduct[]): ICassProduct[] {
  ...
  protected fillComponentAttributesFromResponseBody(data: ICassProduct[] | null): ICassProduct[] {
    return data ?? [];
  }
  ...
  private getEntityKey(item: ICassProduct): string {
    return JSON.stringify(this.cassProductService.getCassProductIdentifier(item));
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.cassProductService.querySlice(queryObject);
  }
