...
import { CassReportDeleteDialogComponent } from '../delete/cass-report-delete-dialog';
import { CassReportService, EntityArrayResponseType } from '../service/cass-report.service';

@Component({
  selector: 'jhi-cass-report',
  templateUrl: './cass-report.html',
  imports: [
    RouterLink,
    FormsModule,
    FontAwesomeModule,
...
export class CassReportComponent implements OnInit {
  ...
  readonly cassReports = signal<ICassReport[]>([]);
  ...
  protected readonly cassReportService = inject(CassReportService);
  ...
  // Saathratri: Single-value Primary Key Code
  trackId = (item: ICassReport): string => this.cassReportService.getCassReportIdentifier(item);
  ...
  delete(cassReport: ICassReport): void {
    const modalRef = this.modalService.open(CassReportDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.cassReport = cassReport;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType, append = false): void {
    ...
    if (append) {
      const current = this.cassReports();
      ...
      this.cassReports.set(this.refineData([...current, ...newItems]));
    } else {
      this.cassReports.set(this.refineData(dataFromBody));
    }
    if (this.hasNextPage && this.totalItems !== null) {
      this.hasNextPage = this.cassReports().length < this.totalItems;
    }
  }

  protected refineData(data: ICassReport[]): ICassReport[] {
  ...
  protected fillComponentAttributesFromResponseBody(data: ICassReport[] | null): ICassReport[] {
    return data ?? [];
  }
  ...
  private getEntityKey(item: ICassReport): string {
    return JSON.stringify(this.cassReportService.getCassReportIdentifier(item));
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.cassReportService.querySlice(queryObject);
  }
